...
const FALLBACK_IMAGE_WIDTH = 200;
...
async function handler(event, context) {
  ...
  // e.g. /https%3A%2F%2Fwww.11ty.dev%2F/jpeg/
  // e.g. /https%3A%2F%2Fwww.11ty.dev%2F/jpeg/400/
  // e.g. /https%3A%2F%2Fwww.11ty.dev%2F/jpeg/400x200/
  let pathSplit = event.path.split("/").filter(entry => !!entry);
  let [url, imageFormat, dimensions] = pathSplit;
  ...
  if(dimensions && dimensions.startsWith("_")) {
  ...
  try {
    console.log("Looking at ", url, width);
    if(!isFullUrl(url)) {
    ...
    if(!urlObj.origin ||
      !urlObj.origin.endsWith(".netlify.com") &&
      !urlObj.origin.endsWith(".netlify.app") &&
      !url.startsWith("https://cdn.sanity.io/images/o0o2tn5x/")) {
      throw new Error(`Invalid \`url\` (only .netlify.com, .netlify.app, cdn.sanity.io URLs allowed)`);
    }

    let stats = await EleventyImage(url, {
      widths: [width || "auto"],
    ...
    return {
      ...
      body: `<svg xmlns="http://www.w3.org/2000/svg" width="${width || FALLBACK_IMAGE_WIDTH}" height="${height || FALLBACK_IMAGE_HEIGHT}" viewBox="0 0 227 120">
  <g fill="#bbbbbb" fill-rule="evenodd" transform="translate(40,40)">
